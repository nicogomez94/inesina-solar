...
$query_obras_fotos = "SELECT * FROM obras_fotos where id_obra = ".intval($_GET['id']);
...
<?php
$query_ico = "SELECT iconos.* FROM trans_iconos inner join iconos on trans_iconos.id_icono = iconos.Id where id_obra = ".intval($_GET['id'])." order by iconos.orden ";
...
<?php
$query_obras = "SELECT * FROM obras where Id != ".intval($_GET['id'])." and destacado = 'Y' order by orden asc limit 3";
$obras = mysqli_query($inesina, $query_obras) or die(mysqli_error());
$row_obras = mysqli_fetch_assoc($obras);
if(mysqli_num_rows($obras)){
?>	       
